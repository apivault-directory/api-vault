      ...
      <section className="max-w-[1280px] mx-auto px-6 py-16">
        ...
        {stats.unknown === providers.length && (
          <p className="text-xs text-fg-2 font-mono mb-8">
            ⚠ Health checks have not run yet — status will update after the first <code>npm run health-check</code>.
          </p>
        )}
        {stats.unknown < providers.length && (
          <p className="text-xs text-fg-2 font-mono mb-8">
            Updated by automated health checks. Run <code>npm run health-check</code> locally or via GitHub Actions.
          </p>
        )}
        <div className="grid grid-cols-4 gap-3 mb-8">
          ...
          <div className="bg-bg-1 border border-line rounded-lg p-5">
            <div className="flex items-center gap-2 text-xs font-mono text-fg-2 uppercase tracking-wider mb-2"><StatusDot status="unknown" size="md" />Not checked</div>
